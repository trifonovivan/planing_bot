...
Pipeline создания задачи:

1. Telegram принимает обычный текст.
2. Go вызывает parser: ML parser через `ML_PARSER_URL` или rule-based fallback.
3. Результат проходит post-parse validation.
4. Validator возвращает decision: `accept`, `accept_with_warnings`, `needs_clarification` или `reject`.
5. При `needs_clarification` бот спрашивает пользователя и не создает задачу молча. Минимальные быстрые действия: `Создать без срока`, `Убрать напоминание`.
6. При `accept*` создается задача, reminder и event payload.

В event payload для `created` сохраняются parser metadata:
...
- `warnings`
- `validation`
- `shadow_parse`, если включен shadow parser

Raw text не пишется в event payload по умолчанию. Вместо него сохраняются `redacted_input_text` и `redacted_task_text`: email, URL, телефоны и token-like строки заменяются плейсхолдерами. Для локального сбора real-world golden examples можно временно включить `PARSER_COLLECT_RAW_TEXT=true`, выгрузить события `created`, после чего выключить флаг обратно. Для evaluation лучше использовать redacted-поля и JSONL-строки вида:

```json
{"input_text":"...","parsed_output":{},"warnings":[],"validation":{"decision":"accept"}}
```

Shadow mode включается через `SHADOW_PARSER_URL`. Основной результат и поведение пользователя не меняются: альтернативный parser только выполняется рядом, а результат попадает в `shadow_parse` и метрики `parser_shadow_total` / `parser_shadow_duration_seconds`.

Новые parser quality metrics:

- `parser_accept_total`
- `parser_reject_total`
- `parser_clarification_total`
- `parser_fallback_total`
- `parser_validation_error_total`
- `parser_validation_warning_total`
- `parser_field_confidence`
- `parser_rule_override_total`
